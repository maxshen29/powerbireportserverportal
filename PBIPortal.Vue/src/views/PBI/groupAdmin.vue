  ...
  <div class="menu-container">
    <div class="menu-left">
      <div class="menu-tree" style="height:95%;margin-bottom:100px">
        <el-input placeholder="搜索组" v-model="filterText"></el-input>
    ...
    <div class="menu-right">
      <div class="rightAll">
        <div class="head">
          <span class="project-name">Power BI 用户组管理</span>
        </div>
        <div class="align-center-vertical" id="ContentText">Power BI用户组管理</div>
        <div class="contentTable" id="Content">
          <VolHeader icon="md-apps" :text="selectGroupName" style="margin-bottom: 20px;">
            <div style="margin-bottom:10px;margin-top:10px;margin-left:10px;text-align: right">
              <Button type="info" ghost @click="addUser">添加用户</Button>
              <Button type="info" ghost @click="delUser">删除用户</Button>
            </div>
          </VolHeader>
    ...
    <vol-box :model.sync="viewModel" :height="800" :width="600" :title="selectGroupName">
      ...
      <div slot="footer">
        <Button type="info" @click="addGroupUser">确认</Button>
        <Button type="info" @click="viewModel=false">取消</Button>
      </div>
    </vol-box>

    <vol-box :model.sync="viewModelGroup" :height="200" :width="600" title="添加用户组">
      <VolForm ref="myform" :loadKey="loadKey" :formFileds="formFileds" :formRules="formRules"></VolForm>
      <div slot="footer">
        <Button type="info" @click="addGroups">确认</Button>
        <Button type="info" @click="viewModelGroup=false">取消</Button>
      </div>
    </vol-box>

    <vol-box :model.sync="viewModelDelGroup" :height="400" :width="600" title="删除用户组">
      <vol-table
        ref="tableGroup"
        :loadKey="true"
      ...
      ></vol-table>
      <div slot="footer">
        <Button type="info" @click="DelGroups">删除选择用户组</Button>
        <Button type="info" @click="viewModelDelGroup=false">取消</Button>
      </div>
    </vol-box>
...
export default {
  components: { VolTable, VolBox, VolForm, VolHeader, UploadExcel },
  watch: {
    filterText(val) {
  ...
  methods: {
    ...
    DelGroups() {
      ...
      if (this.$refs.table.rowData.length > 0) {
    ...
    loadTableBefore(param, callBack) {
      //此处是从后台加数据前的处理，自己在此处自定义查询条件,查询数据格式自己定义或参考代码生成器查询页面请求的数据格式
      param.wheres = [{ name: "pid", value: this.pid, displayType: "text" }];
      //  //////console.log(param);
      // //////console.log("加载数据前" + param);
      callBack(true); //此处必须进行回调，返回处理结果，如果是false，则不会执行后台查询
    },
    ...
    load() {
      ...
      // let where={wheres:[{"name":"UserTrueName","value":"教兽",displayType:"text"}]};
      let where = {
        wheres: [{ name: "pid", value: this.pid, displayType: "text" }]
      };
