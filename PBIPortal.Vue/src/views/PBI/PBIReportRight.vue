<template>
  <div class="menu-container">
    ...
    <div class="menu-right">
      <div class="rightAll">
        <div class="head">
          <span class="project-name">Power BI 报表权限管理</span>
        </div>
        <div class="align-center-vertical" id="ContentText">Power BI 报表权限管理</div>
        <div class="contentTable" id="Content">
          <VolHeader icon="md-apps" :text="selectGroupName" style="margin-bottom: 20px;">
            <div style="margin-bottom:10px;margin-top:10px;margin-left:10px;text-align: right">
              <Button type="info" ghost @click="addReportRight">添加报表组权限</Button>
              <Button type="info" ghost @click="delReportRight">删除报表组权限</Button>
              <Button type="info" ghost @click="setReportRight">设置报表服务器权限</Button>
            </div>
          </VolHeader>
          ...
          <VolHeader icon="md-apps" :text="selectUserName" style="margin-bottom: 20px;">
            <div style="margin-bottom:10px;margin-top:10px;margin-left:10px;text-align: right">
              <Button type="info" ghost @click="addReportUserRight">添加报表用户权限</Button>
              <Button type="info" ghost @click="delReportUserRight">删除报表用户权限</Button>
            </div>
          </VolHeader>
          <div style="margin-bottom:10px;margin-top:10px;margin-left:10px;">
            <vol-table
...
export default {
  ...
  data() {
    ...
    return {
      ...
      urlCatalogItems: "api/V_Group_Report_Right/getPageData", //后从加载数据的url
      urlGroup: "api/PBI_SSO_Group/getPageData", //后从加载数据的url
      urlUser: "api/v_User_Report_Right/getPageData", //后从加载数据的url
  ...
  methods: {
    ...
    addUserRight() {
      ...
      this.http
        .post("/api/PBI_User_Right/InsertUserRight", postdata, true)
        .then(x => {
          if (!x.status) {
            return this.$message.success(x.message);
          }
    ...
    delReportUserRight() {
      ...
      let url = "/api/PBI_User_Right/Del";
    ...
    delReportRight() {
      ...
      let url = "/api/PBI_Group_Right/Del";
      // let url = this.getUrl(this.const.DEL);
      this.http.post(url, delKeys, "正在删除数据....").then(x => {
        if (!x.status) return this.$message.error(x.message);
        this.$message.error(x.message);
    ...
    addReports() {
      ...
      this.http
        .post("/api/PBI_Group_Right/InsertGroupRight", postdata, true)
        .then(x => {
          if (!x.status) {
            return this.$message.success(x.message);
          }
    ...
    appendCatalog(data) {
      //  console.log(data);
      this.selectGroupName = data.label + "--报表组权限管理";
      this.selectUserName = data.label + "--报表用户权限管理";
      if (data.type == "folder") {
