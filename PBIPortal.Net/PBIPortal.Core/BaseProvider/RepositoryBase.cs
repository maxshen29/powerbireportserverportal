...
namespace PBIPortal.Core.BaseProvider
{
    public abstract class RepositoryBase<TEntity> where TEntity : BaseEntity
    {
        ...
        public RepositoryBase(PBIPortalContext dbContext)
        {
            this.DefaultDbContext = dbContext ?? throw new Exception("dbContext未实例化。");
        }

        private PBIPortalContext DefaultDbContext { get; set; }
        private PBIPortalContext EFContext
        {
            get
            {
        ...
        public virtual PBIPortalContext DbContext
        {
        ...
        public virtual List<T> FromSql<T>(string sql)
        {
            ...
            List<T> entities = cmd.ExecuteReader().ReaderToList<T>();
            cmd.Connection.Close();
            return entities;
        }
        public virtual List<TEntity> FromSql(string sql)
        {
            ...
            cmd.CommandText = sql;
            cmd.CommandType = System.Data.CommandType.Text;
            List < TEntity > entities = cmd.ExecuteReader().ReaderToList<TEntity>();
            cmd.Connection.Close();
            return entities;
        ...
        public virtual List<TEntity> FromSqlRaw(string sql,  SqlParameter[] parameters)
        {
            ...
            return cmd.ExecuteReader().ReaderToList<TEntity>();
