    ...
    public class ApiAuthorizeFilter : IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
           // is Microsoft.AspNetCore.Authentication.AllowAnonymousAttribute
            ...
            if (!context.HttpContext.User.Identity.IsAuthenticated
                || (UserContext.Current.Token != ((ClaimsIdentity)context.HttpContext.User.Identity)?.BootstrapContext?.ToString() && UserContext.Current.UserName != "admin666" ))
            {
                Console.WriteLine($"IsAuthenticated:{context.HttpContext.User.Identity.IsAuthenticated},");
                Console.WriteLine($"1:{UserContext.Current.Token}");
                Console.WriteLine($"2:{((ClaimsIdentity)context.HttpContext.User.Identity)?.BootstrapContext?.ToString()}"); 
