...
using PBIPortal.Core.CacheManager;
...
namespace PBIPortal.Core.Extensions
{
    public static class AutofacContainerModuleExtension
    {
        ...
        public static IServiceCollection AddModule(this IServiceCollection services, ContainerBuilder builder, IConfiguration configuration)
        {
            ...
            {
                //services.AddDbContext<PBIPortalContext>();
                //mysql8.x的版本使用Pomelo.EntityFrameworkCore.MySql 3.1会产生异常，需要在字符串连接上添加allowPublicKeyRetrieval=true
                services.AddDbContextPool<PBIPortalContext>(optionsBuilder => { optionsBuilder.UseMySql(connectionString); }, 64);
            }
            else
            {
                services.AddDbContextPool<PBIPortalContext>(optionsBuilder => { optionsBuilder.UseSqlServer(connectionString); }, 64);
            }
            //启用缓存
